...
module.exports = {
    ...
    userSignUp: (req, res) => {
        ...
        bcrypt.hash(data.password, saltRounds).then(hash => {
            data.password = hash;
            models.regisUser(data)
                .then(result => {
                    if(role == 'company') {
                        log('comp');
                        models.addCompany(ue)
                            .then(result => response.ok(res, ue, 'Register successfully', 201))
                            .catch(err => {
                        ...
                        engineer = {created: lib.date(), updated: lib.date(), ...ue};
                        models.addEngineer(engineer)
                            .then(result => response.ok(res, engineer, 'Register successfully', 201))
                            .catch(err => {
                                models.deleteUser(id)
                                    .then(() => response.err(res, err, 'Ops, register failed'))
                                    .catch(err => response.err(res, err, 'Ops, register failed'))
